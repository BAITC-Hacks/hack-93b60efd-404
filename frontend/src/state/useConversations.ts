...
const KEY = 'voice-router.conversations.v1';
...
export function useConversations() {
  ...
  useEffect(() => {
    const strip = conversations.map((c) => ({
      ...c,
      messages: c.messages.map((m) => (m.turn?.audio_b64 ? { ...m, turn: { ...m.turn, audio_b64: null } } : m)),
    }));
  ...
  return { conversations, active, activeId, setActiveId, create, append, patch, remove, clearAll };
}
