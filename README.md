...
Requirements: Python 3.12, [uv](https://docs.astral.sh/uv/), Node.js 20+, npm, and server-side `OPENAI_API_KEY`, `GEMINI_API_KEY`, and `TYPESAFE_API_KEY`. Never put keys in `frontend/.env`, browser code, or Git.

```sh
bash scripts/run-local.sh
```

The script installs locked Python and frontend dependencies, starts the backend at `127.0.0.1:8000`, and serves the app at [http://localhost:5173](http://localhost:5173). Allow microphone access in the browser. Speak naturally in Russian, Kazakh, or a mixture; text is a supplementary input. The assistant voice is AI-generated. Open the trace icon after a turn to inspect the chosen scenario, alternatives, rationale, actions, and stage timings. The sidebar contains conversation history and the supervisor panel.
...
Gemini 3.8 Live handles streaming microphone input, natural RU/KK dialogue, spoken output, and interruption. Insurance requests call the local backend through a Live tool. OpenAI `gpt-6-luna` selects an ordered route from the 40 official scenarios using dialogue context; a Jev 1.13 yes/no decision skips Luna slot extraction only when no scenario-relevant value was explicitly spoken. Otherwise Luna extracts the values. Case actions run under approval rules and return verified results to Gemini Live for speech. A backend session holds at most ten customer turns. The browser displays the real route trace, including Jev timing and whether extraction was skipped. The separate HTTP audio endpoint remains available for backend diagnostics.
...
The supplied records, prices, and policies are synthetic. Actions and handoff requests exist only in the local server process; no payment, SMS, live operator connection, or real booking system is attached. Irreversible changes require a distinct interpreted confirmation turn. A phone match in the supplied records is not production-grade customer authentication. Sessions and generated speech disappear on server restart. This local service has no public authentication or rate limit and must **not** be exposed to the internet as-is. There is no verified public deployment.
